...
import type { ViajeVentaTransporteConContext } from "@/app/types/viajeVentaTransporte";
...
/** Lote corto para línea de venta (misma idea que proveedor sin SKU en línea). */
...
function fechaIngresoTransporte(v: ViajeVentaTransporteConContext): string {
  const f = strTrim(v.ventaFecha);
  if (f) return f;
...
/** Mismas columnas que proveedor / listados de cuenta: RD … Estado */
export type FilaTransporteInventario = {
  id: string;
  rd: string;
  renglon: number;
  lote: string;
...
function filasDesdeViajes(
  viajes: ViajeVentaTransporteConContext[],
  catalogById: Map<string, Catalogo>,
  catalogos: Catalogo[],
): FilaTransporteInventario[] {
  const out: FilaTransporteInventario[] = [];
  for (const v of viajes) {
    const items = v.lineItemsEsperados ?? [];
    items.forEach((li, idx) => {
      ...
      const marca = strTrim(v.ventaCompradorNombre) || "—";
      const embalaje =
        (cat?.logisticService && String(cat.logisticService).trim()) ||
        (cat?.inventoryTracker && String(cat.inventoryTracker).trim()) ||
        "";

      const estadoVenta = strTrim(v.ventaEstado) || "Transporte";

      out.push({
        id: `${v.idClienteDueno}-${v.ventaId}-${v.id}-${idx}`,
        rd: strTrim(v.numero) || v.id,
        renglon: idx + 1,
        ...
        caducidad: "",
        fechaIngreso: fechaIngresoTransporte(v),
        llaveUnica: `${v.idClienteDueno}::${v.ventaId}::${v.id}::${idx}`,
        estado: estadoVenta,
      });
    });
  }
...
export default function ListadoCargue() {
  ...
  const filas = useMemo(
    () => filasDesdeViajes(viajes, catalogById, catalogos),
    [viajes, catalogById, catalogos],
  );
  ...
  if (!idCliente.trim() || !codeCuenta.trim()) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">
        Tu sesión no tiene cuenta vinculada. No se puede cargar el inventario en transporte.
      </div>
    );
  }

  return (
    <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white font-['Inter'] shadow-sm">
      <div className="border-b border-slate-100 bg-[#A8D5BA]/20 p-5">
        <h3 className="text-[18px] font-bold uppercase tracking-tight text-slate-800">En inventario transp</h3>
        <p className="mt-1 text-xs text-slate-600">
          Misma grilla que <strong>proveedor</strong>: viajes de venta <strong>En curso</strong> (mercancía en camino).
          La columna <strong>Proveedor</strong> muestra el <strong>comprador</strong> de la orden de venta.
          {filas.length > 0 ? (
            ...
            {showEmpty ? (
              <tr>
                <td colSpan={12} className="px-4 py-8 text-center text-sm text-slate-500">
                  No hay viajes en curso para esta cuenta. Cuando una venta pase a «Transporte» y exista un viaje
                  pendiente de entrega, las líneas aparecerán acá.
                </td>
              </tr>
            ) : null}
              ...
              ? pageItems.map((d) => (
                  <tr key={d.id} className="transition-colors hover:bg-slate-50/80">
                    ...
                    <td
                      ...
                      title={d.descripcion}
                    >
                      <span className="inline-block max-w-full truncate rounded-lg bg-slate-100 px-3 py-1.5 text-[13px] font-semibold text-slate-700">
                        {d.descripcion}
                      </span>
                    </td>
      ...
      {filas.length > 0 ? (
        ...
        >
          <div className="flex flex-col items-stretch sm:flex-row sm:items-center sm:justify-end sm:gap-4">
            <span className="text-center text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 sm:pt-1 sm:text-right">
