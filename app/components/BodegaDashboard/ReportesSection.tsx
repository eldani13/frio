...
import { MdBarChart, MdMoveToInbox, MdLogout, MdInbox, MdLocalShipping, MdClose } from "react-icons/md";
...
interface ReportesSectionProps {
  ...
  reportDetailModal: { type: "ingresos" | "salidas" | "movimientos" | "despachados" } | null;
  setReportDetailModal: (
    modal: { type: "ingresos" | "salidas" | "movimientos" | "despachados" } | null,
  ) => void;
}
...
const ReportesSection: React.FC<ReportesSectionProps> = ({
  ...
  const { ingresos, salidas, movimientosBodega } = useBodegaHistory();
  ...
  const globalSalidas = salidas.length > 0 ? salidas : orders.filter(
  ...
  <section className="rounded-2xl bg-white p-6 shadow-sm">
    ...
    <div className="mt-6 grid gap-8 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="flex-1 flex items-center justify-center">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={reportData} barSize={32}>
              ...
              <Tooltip contentStyle={{ borderRadius: 12, background: '#fff', border: '1px solid #e5e7eb', color: '#334155' }} cursor={{ fill: '#6366f11a' }} />
      ...
      <div className="rounded-3xl border border-slate-200 bg-linear-to-br from-slate-50 to-white p-6 shadow-sm flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
            <svg width="20" height="20" fill="none" viewBox="0 0 20 20"><circle cx="10" cy="10" r="10" fill="#10B981"/><path d="M10 4v6l5 3" stroke="#fff" strokeWidth="2" strokeLinecap="round"/></svg>
            Distribución
          </h3>
        ...
        <div className="flex-1 flex items-center justify-center">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Tooltip contentStyle={{ borderRadius: 12, background: '#fff', border: '1px solid #e5e7eb', color: '#334155' }} />
              <Pie
                data={reportData}
                dataKey="value"
                nameKey="name"
    ...
    <div className="mt-8 grid gap-4 sm:grid-cols-2 md:grid-cols-4">
      ...
      >
        <MdInbox size={32} className="text-blue-500 mb-2" />
      ...
      <button
        ...
        className="rounded-2xl border border-slate-100 bg-white p-5 flex flex-col items-center hover:shadow-md transition w-full"
      >
        <MdLogout size={32} className="text-rose-500 mb-2" />
      ...
      <button
        ...
        className="rounded-2xl border border-slate-100 bg-white p-5 flex flex-col items-center hover:shadow-md transition w-full"
      >
        <MdMoveToInbox size={32} className="text-amber-500 mb-2" />
        <span className="text-xs font-semibold uppercase text-slate-500">Movimientos a bodega</span>
        <span className="mt-1 text-2xl font-bold text-slate-900">
      ...
      <button
        ...
        className="rounded-2xl border border-slate-100 bg-white p-5 flex flex-col items-center hover:shadow-md transition w-full"
      >
        <MdLocalShipping size={32} className="text-green-500 mb-2" />
                        ...
                        <ul className="grid gap-3 md:grid-cols-2">
                          {sortByPosition(globalIngresos).map((box, idx) => (
                            ...
                            >
                              ...
                              <span className="text-slate-900 font-medium">{box.name}</span>
                              <span className="text-xs text-slate-500">Temp: {box.temperature} °C</span>
                              <span className="text-xs text-slate-400">ID: {box.autoId}</span>
                            </li>
                          ))}
                        </ul>
                      );
                          ...
                          {globalSalidas.map((order) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="text-slate-900 font-medium">ID orden: {order.id}</span>
                                <span className="text-xs text-slate-500">Por: {order.createdBy}</span>
                                <span className="text-xs text-slate-500">Fecha: {order.createdAt}</span>
                                <span className="text-xs text-slate-500">Origen: {order.sourcePosition ?? "-"}</span>
                                <span className="text-xs text-slate-500">Destino: {order.targetPosition ?? "-"}</span>
                                {box ? (
                                  <>
                                    <span className="text-xs text-slate-400">{box.name} | Temp: {box.temperature} °C | ID: {box.autoId}</span>
                                  </>
                                ) : (
                            ...
                            return (
                              ...
                              >
                                <span className="font-semibold text-amber-600 text-sm mb-1">
                                  Movimiento a bodega {order.targetPosition ?? "-"}
                                </span>
                                <span className="text-slate-900 font-medium">ID orden: {order.id}</span>
                                <span className="text-xs text-slate-500">Por: {order.createdBy}</span>
                                <span className="text-xs text-slate-500">Fecha: {order.createdAt}</span>
